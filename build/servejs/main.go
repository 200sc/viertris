...
func run() error {
	...
	http.Handle("/", http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		...
		case "/favicon.ico":
			...
			return
		default:
			// Two requests fallthrough: directories and directory/viertris_jswasm.wasm
			isWasmRequest := strings.HasSuffix(r.URL.Path, "viertris_jswasm.wasm")
			if !isWasmRequest {
				r.URL.Path = filepath.Join(r.URL.Path, "viertris_jswasm.wasm")
			}
			...
			if err != nil {
				...
				rw.Write([]byte(
					fmt.Sprintf("cd %v\n", subDir)))
				rw.Write([]byte(`(powershell) $Env:GOOS="js"; $Env:GOARCH="wasm"; go build -o viertris_jswasm.wasm .`))
				rw.Write([]byte("\n"))
				rw.Write([]byte("(unix)       GOOS=js GOARCH=wasm go build -o viertris_jswasm.wasm .\n"))
